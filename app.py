...
async def cloneMessage(message, channel_id, hook_id):
    ...
    embed.set_author(name=f'{message.author.name}#{message.author.discriminator}', icon_url=message.author.avatar_url if hasattr(message.author, 'avatar_url') else None)
